...
//	(非同期化ONが保存できない様子はユーザからみておかしな動きに見えるため)
	...
	{
        //#define ID3V2_PADDING_SIZE 16 //文字列を少しずつ長くしていってパディングを挿入していっても問題ないことの確認用
		//[パディング領域を挿入]
        ...
        //ヘッダの分を考慮
        DWORD dwPaddingSize = (ID3V2_PADDING_SIZE + 10 + dwTotalFrameSize - 1) / ID3V2_PADDING_SIZE * ID3V2_PADDING_SIZE - dwId3Size;
        if(dwId3Size == 0){//ID3v2 がない
            dwId3Size = dwPaddingSize - 10;//ヘッダの分を差し引く
                                           //挿入すべきパディングのサイズそのものは変更なし
        }
        else{//既に ID3v2 ヘッダがあるので追加分のサイズは 10 バイト少ない
            dwPaddingSize -= 10;
            dwId3Size += dwPaddingSize;
        }
		//==================元ファイルをメモリに保存==================
